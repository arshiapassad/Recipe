...
import academy.nouri.s4_recipeapp.adapter.InstructionsAdapter
import academy.nouri.s4_recipeapp.adapter.SimilarAdapter
...
import com.example.recipe.utils.Constants.setTint
    ...
    private fun loadSimilarData() {
        binding.apply {
            viewModel.similarData.observe(viewLifecycleOwner) { response ->
                when (response) {
                    is NetworkRequest.Loading -> {
                        loadingSimilar.isVisible = true
                    }

                    is NetworkRequest.Success -> {
                        loadingSimilar.isVisible = false
                        response.data?.let { data ->
